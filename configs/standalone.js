module.exports = function(config, optimist) {
  ...
  if (!optimist.local) {
    optimist
      ...
      .describe(
        'b',
        'Start the bridge server - to receive commands from the cli'
      )
      ...
      .describe(
        'inProcessLocalFs',
        'Whether to run localfs in same process for debugging.'
      )
      .default('inProcessLocalFs', config.inProcessLocalFs)
      .boolean('useBrowserCache')
      .describe(
        'useBrowserCache',
        'Use window.caches api if available for faster loading, requires https://'
      )
      .describe(
        'secure',
        'path to file containing ssl certificate (can be generated using scripts/create-cert.sh)'
      )
  }
  ...
  if (argv.help) return null

  var testing = argv.t
  var baseProc = path.normalize(
    testing
  ...
  if (argv.collab != null) config.collab = argv.collab

  var workspaceType = argv.workspacetype || null

  if (argv.hosted) config.client_config = 'default-hosted'

  if (!argv.hosted) config.sourceDir = path.dirname(__dirname)
  ...
  if (!config.startBridge) config.startBridge = startBridge

  if (testing && argv.k)
    require('child_process').exec(
      'tmux -L cloud91.9 kill-server',
      function() {}
    )
...
if (!module.parent)
  require('../server')([__filename].concat(process.argv.slice(2)))
